...
class FrontendUserController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController {
	...
	public function createAction(FrontendUser $newFrontendUser) {
		...
		$existingFrontendUser = $this->frontendUserRepository->findOneByEmailAndStoragePageId($newFrontendUser->getEmail(), (int)$this->settings['userFolder']);
		if ($existingFrontendUser instanceof FrontendUser) {
			if ($existingFrontendUser->isDisable() === TRUE) {
				...
				$emailContent = LocalizationUtility::translate('createFrontendUser.optInEmail.activate', $this->controllerContext->getRequest()->getControllerExtensionName(), array(1 => $this->settings['newsletterTitle'], 2 => $optInUri));
				...
				if (!empty($this->settings['fieldList'])) {
					...
					$emailContent = LocalizationUtility::translate('createFrontendUser.optInEmail.edit.unsubscribeOnly', $this->controllerContext->getRequest()->getControllerExtensionName(), array(1 => $this->settings['newsletterTitle'], 2 => $optInUri));
					$this->sendEmail($existingFrontendUser->getEmail(), $this->settings['newsletterTitle'], $emailContent);
				} else {
	...
	public function activateAction($ruid = '', $verify = '') {
		...
		if (!$frontendUserUid) {
			...
			if ($verify === GeneralUtility::stdAuthCode($frontendUserUid)) {
				$frontendUser = $this->frontendUserRepository->findOneByUidAndStoragePageId($frontendUserUid, (int)$this->settings['userFolder']);
				if ($frontendUser instanceof FrontendUser) {
					$frontendUser->setDisable(FALSE);
					$this->frontendUserRepository->update($frontendUser);
					$this->persistenceManager->persistAll();
					$successMessage = LocalizationUtility::translate('activateFrontendUser.success', $this->controllerContext->getRequest()->getControllerExtensionName(), array(1 => $this->settings['newsletterTitle']));
			...
			if ($verify === GeneralUtility::stdAuthCode($frontendUserUid)) {
				...
				if ($frontendUser instanceof FrontendUser) {
					...
					if (!empty($this->settings['fieldList'])) {
						...
						$this->view->assign('fieldsToRender', GeneralUtility::trimExplode(',', $this->settings['fieldList']));
					}
				} else {
					// no frontend user found
					$this->forward('invalidLink');
	...
	public function updateAction(FrontendUser $frontendUser, $verify = '') {
		if ($verify === GeneralUtility::stdAuthCode($frontendUser->getUid())) {
			$this->frontendUserRepository->update($frontendUser);
			$this->persistenceManager->persistAll();
			$successMessage = LocalizationUtility::translate('updateFrontendUser.success', $this->controllerContext->getRequest()->getControllerExtensionName());
			$this->addFlashMessage($successMessage);
			$this->redirect('edit', NULL, NULL, array('verify' => $verify, 'ruid' => base64_encode($frontendUser->getUid())));
		} else {
	...
	protected function createOptInUri($uid, $action = 'activate') {
		$arguments = array(
			array('tx_newsletterregistration_newsletterregistration' =>
				array(
	...
	protected function sendEmail($email, $subject, $content, $replyTo = '', $returnPath = '') {
		...
		$message->addTo($email);
		$message->setFrom(array($this->settings['sender']['email'] => $this->settings['sender']['name']));
